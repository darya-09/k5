...
- 🔴 **ПОДТВЕРЖДЕНО (по `tpe/optimizer/tpe.py`):** `weight_fn` получает среднее координат `mean(x0,x1)`, а ноутбук считает ∇f в точке (mean,0) → метод 2 НЕ реализует градиентное взвешивание; его grad_*-результаты невалидны как «gradient-aware».
...
(5) метод 2 (w(x)) реализован с дефектом — weight_fn получает среднее координат, градиент берётся в точке (mean,0),
значит grad_*-результаты нельзя трактовать как градиентное взвешивание.
**Нельзя пока:** утверждать пользу/вред w(x) по градиенту (метод надо исправить); пользу нормализации при шуме; объяснять gTPE одним фактором.
...
5. ✅ Сделано: пакет `tpe/` получен (в ветке `main`), вызов `weight_fn` проверен — см. п.8 (метод 2 требует исправления).
