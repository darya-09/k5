...
- **🔴 ГЛАВНЫЙ РИСК (отмечен в самом коде, `_as_2d_points`):** неизвестно, как repo-`TPEOptimizer` вызывает `weight_fn` — по полным 2D-точкам или **покоординатно**. При покоординатном вызове код подставляет x1=0 → считается НЕ тот градиент → grad-логика превращается в шум. **Требует проверки исходника `tpe`.**
- **🟠 Второй риск:** используется ТОЧНЫЙ ∇f benchmark-а → это «белый ящик», а не реальный black-box HPO.
- **Выводы МОЖНО:** только после проверки вызова weight_fn; пока — «эффект w(x) мал/неустойчив».
- **Выводы НЕЛЬЗЯ:** «w(x) улучшает TPE» без (а) проверки 2D-вызова и (б) стат-теста по seeds.
