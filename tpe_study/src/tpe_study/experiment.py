...
поэтому любая пара "tpe vs tpe_*" отличается ровно одним фактором):
    ...
    tpe_w_*         — TPE + градиентный вес КАНДИДАТА, 4 формы (фактор: weight, white-box)
    tpe_refine      — TPE + локальный градиентный спуск по кандидату (white-box)
    tpe_gp          — TPE + GP-переранжирование кандидатов (black-box: ∇f не используется)
    tpe_gp_w        — GP + вес наблюдений 1/‖∇f‖ (аналог gТPE из fin_5)
    tpe_gp_refine   — GP + refinement
    optuna          — внешний референс TPE
См. ALGO_FAMILY для классификации black-box / white-box(∇f).
