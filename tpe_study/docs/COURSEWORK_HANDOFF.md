...
3. **Ни одна из 4 форм w(x) не даёт статистически значимого улучшения** над baseline — даже при точном градиенте. По средним `tpe_w_smooth` «лучший» (87.5% ячеек), но это не переживает поправку на множественность (0 значимых) — средние обманывают.
