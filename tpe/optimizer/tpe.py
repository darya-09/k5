...
from tpe.weight_func import WeightFn, compute_log_weight
...
class TreeStructuredParzenEstimator:
    ...
    def compute_probability_improvement(self, config_cands: dict[str, np.ndarray]) -> np.ndarray:
        ...
        if self._weight_fn is not None:
            # Берём только численные параметры (не категориальные)
            numerical_arrays = [
                v.astype(float)
                for hp_name, v in config_cands.items()
                if not self._is_categoricals[hp_name]
            ]

            if len(numerical_arrays) == 0:
                # Все параметры категориальные — w(x) не применяем
                log_w = 0.0
            else:
                # Усредняем по всем численным измерениям
                # shape: (n_ei_candidates, n_numerical_params) -> (n_ei_candidates,)
                samples = np.stack(numerical_arrays, axis=1).mean(axis=1)
                log_w = compute_log_weight(
                    x=samples,
                    weight_fn=self._weight_fn,
                    low=self._weight_low,
                    high=self._weight_high,
                )
        else:
            log_w = 0.0

        first_term = np.log(self._percentile + EPS)
        second_term = np.log(1.0 - self._percentile + EPS) + cll_upper - cll_lower - log_w
